...
async function getDailyScheduleOfGroup(
  ...
  const rangeCursuri =
    an === 3 && seria === "G"
      ? `${sheetName}!${horizontalStartCursuri}${verticalStart}:${horizontalStartCursuri}${verticalEnd}`
      : `${sheetName}!${horizontalStart}${verticalStart}:${horizontalStart}${verticalEnd}`;
  ...
  if (valuesCursuri !== undefined)
    intervaleCursuri = parseCursuri(
      valuesCursuri,
      saptamana,
      ore.data.values.map((subArray) => subArray[0]),
    );
  //console.log("Intervale cursuri: " + JSON.stringify(intervaleCursuri));
  if (values !== undefined)
    intervaleLaboratoare = parseLabs(
      values,
      saptamana,
      ore.data.values.map((subArray) => subArray[0]),
    );
...
function parseLabs(laboratoare, saptamana, ore) {
  ...
  for (let i = 0; i < laboratoare.length; i++) {
    ...
    if (
      materieOra.length === 0 ||
      materieOra[0].includes("curs") ||
      materieOra[0].includes("lecture") ||
      materieOra[0].includes("(c)") ||
      regexGrupa.test(materieOra)
    )
      continue;
    ...
    if (regexLinieNoua.test(semigrupa)) {
      const randuriElement = semigrupa
        .split(/[\r\n]+/)
      ...
      if (!semigrupa.includes("/")) {
        ...
        materie =
          saptamana === "impar"
            ? randuriElement[0].split("/")[0].trim()
            : randuriElement[0].split("/")[1].trim();
        ...
        sala =
          saptamana === "impar"
            ? randuriElement[1].split("/")[0].trim()
            : randuriElement[1].split("/")[1].trim();
      } else {
        const newText =
          saptamana === "impar"
            ? randuriElement[0].replace("/", "").trim()
            : randuriElement[1].replace("/", "").trim();
        const lastCharactersArray = newText.split(" ");
        const lastCharacters =
          lastCharactersArray[lastCharactersArray.length - 1];
        if (
          ...
          sala = lastCharacters;
          materie = newText
            .substring(0, newText.indexOf(lastCharacters))
            .trim();
        } else {
          materie = newText;
          sala = "--";
    ...
    const timeRange = threeHoursLabs.some(
      (element) => element === materieTrimmed,
    )
      ? extendTimeRange(ore[i], 2)
      : fourHoursLabs.some((element) => element === materieTrimmed)
        ? extendTimeRange(ore[i], 3)
    ...
    if (materie.length !== 0 && !materie.includes("--")) {
      ...
      if (
        laboratoare[i + 1] &&
        laboratoare[i + 1].length !== 0 &&
        laboratoare[i + 1][0].includes(materieTrimmed)
      )
        i += 2;
      else if (
        laboratoare[i + 2] &&
        laboratoare[i + 2].length !== 0 &&
        laboratoare[i + 2][0].includes(materieTrimmed)
      )
        i += 3;
    }
  }
...
async function getSheetColumnsAndLinesNumberForMultipleSheets(
  auth,
  spreadSheetId,
  sheetNames,
) {
...
async function searchValuesInAllSheets(
  auth,
  sala,
  columnsCounts,
  seriesRanges,
  ...
  const fetchDayRanges = async () => {
    for (const sheet of targetSheets) {
      ...
      if (!dayRangesCache[an]) {
        dayRangesCache[an] = await getDayRanges(
          auth,
          spreadsheetId,
          an,
          columnCount,
                ...
                .map((cell, colIndex) => {
                  if (cell.includes(sala)) {
                    for (
                      ...
                      if (
                        row[leftIndex] &&
                        (row[leftIndex].includes("curs") ||
                          row[leftIndex].includes("lecture"))
                      ) {
                        const ora = row[0];
                        return {
...
app.get("/api/orar-profesor", async (req, res) => {
  ...
  try {
    ...
    const sheetColumnsAndLines =
      await getSheetColumnsAndLinesNumberForMultipleSheets(
        client,
        spreadSheetId,
        sheetsNames,
      );
    ...
    const columnsCounts = sheetColumnsAndLines.map(
      (sheet) => sheet.columnCount,
    );

    // Fetch series ranges (assuming getSeriesRange function exists)
    const seriesRange1 = await getSeriesRange(
      client,
      spreadSheetId,
      sheetsNames,
    );
    ...
    const profesorResult = await searchTeacherInAllSheets(
      client,
      profesor,
      columnsCounts,
      seriesRange1,
...
app.get("/api/orar-sala", async (req, res) => {
  ...
  try {
    ...
    // Use the searchValuesInAllSheets function to get the schedule for the entire week
    const salaResult = await searchValuesInAllSheets(
      client,
      sala,
      columnsCounts,
      seriesRange1,
...
async function searchTeacherInAllSheets(
  auth,
  teacher,
  columnsCounts,
  seriesRanges,
