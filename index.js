...
const threeHoursLabs = ["AMP (l)", "CCP (l)", "DE (l)", "ASC (l)", "IEM (l)", "TAPDS (l)"];
...
async function getDailyScheduleOfGroup(auth, spreadSheetId, an, zi, columnCount, grupa, seria, saptamana) {
  ...
  if (valuesCursuri !== undefined) intervaleCursuri = parseCursuri(valuesCursuri, saptamana);
...
function parseLabs(values, saptamana) {
  ...
  const ore = ['08-09', '09-10', '10-11', '11-12', '12-13', '13-14', '14-15', '15-16', '16-17', '17-18', '18-19', '19-20', '20-21'];
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (orarOra.length !== 0 && !orarOra[0].includes("curs")) {
      let timeRange = threeHoursLabs.some(element => orarOra[0].includes(element)) ? extendTimeRange(ore[i], 2) : extendTimeRange(ore[i], 1);
      const checkSemigrupa = orarOra[0].split(',');
      const semigrupa = checkSemigrupa.length === 2 ? checkSemigrupa[1] : checkSemigrupa[0];
      ...
      if (materie.length !== 0 && materie !== '--') intervaleMaterii.push({
        "ora": timeRange,
        "materie": materie,
        "sala": sala
      });
  ...
  for (let i = 1; i < valuesCursuri.length; i++) {
    if (valuesCursuri[i].length > 0) {
      ...
      if (element.includes("(c)") && (element.includes("|") || element.toLowerCase().includes("opt"))) {
        ...
        const timeRange = extendTimeRange(ore[i], 1);
        const newlineRegex = /[\r\n]+/;
        intervaleMaterii.push({"ora": timeRange, "materie": element.replace(newlineRegex, ' ')});
        ...
        if (newlineRegex.test(element)) {
          ...
          } else if (splitNewLines[1].trim()[0] === '/' && saptamana === 'impara') {
          ...
          } else if (splitNewLines[1].trim()[0] === '/' && saptamana === 'para') {
...
app.get("/", async (req, res) => {
  ...
  const {rowCount, columnCount} = await getSheetColumnsAndLinesNumber(client, spreadSheetId, "AN 1");
  const orarSeriaALuni = await getDailyScheduleOfGroup(client, spreadSheetId, 1, "miercuri", columnCount, "412Aa", 'A', "para");
